<?
	...
	// blogtruyen
		...
		$this_chapter = preg_replace('/(.*)(chap-|chapter-|chuong-)(.*)(\/)(.*)/is','$3','http://'.$chapter);
		...
		$img_full = NULL;
		foreach($html->find('textarea[id=txtarea]') as $element){
...
?>
<div class="well">
  <form class="bs-example form-horizontal" method="POST" action="index.php?view=admin&sub_view=grab.comicvn&action=grab_links&token=<?=$_SESSION[token]?>">
  <fieldset>
	  <legend><?=$lang[Grab]?> - ComicVN.net - Bước 1: Grab links của các tập</legend>
	  <div class="form-group">
		<label class="col-lg-2 control-label">URL của bộ truyện</label>
		<div class="col-lg-10">
		  <input type="text" class="form-control" name="url" placeholder="http://comicvn.net/truyen-tranh/dragon-ball/32">
...
<?
	if($_POST && $_GET[token] == $_SESSION[token] && $_GET[action] == 'grab_links'){
		...
		foreach($html->find('div[class=content3]') as $element){
			   $link = $element->find('a');
			   foreach($link as $element) 
			   $img_full = $img_full.'http://comicvn.net'.$element->href.'<br />';
		}
...
?>
<div class="well">
  <form class="bs-example form-horizontal" method="POST" action="index.php?view=admin&sub_view=grab.comicvn&action=grab_contents&token=<?=$_SESSION[token]?>">
	<fieldset>
	  <legend><?=$lang[Grab]?> - ComicVN.net - Bước 2: Grab nội dung từng tập</legend>
	  <div class="form-group">
		<label class="col-lg-2 control-label">URL truyện</label>
		<div class="col-lg-10">
		  <textarea class="form-control" rows="5" name="chapter_list" placeholder="http://comicvn.net/truyen-tranh/glass-mask-mat-na-thuy-tinh/chapter-70/16134"></textarea>
